    ...
    def diff(self, x):
        ...
        for i in range(n): # for all monomials
            for j in range(d): # for all dimensions
                ...
                if tf.reduce_any(e < 0):
                    continue # nothing to do, already zero
                
                y[i, j, :] = a*tf.ones([1, m])
                for k in range(d):
                    y[i, j, :] = y[i, j, :] * tf.math.pow(x[k, :], e[k])
        return y
        ...
        for i in range(n): # for all monomials
            for j1 in range(d): # for all dimensions
                for j2 in range(d): # for all dimensions
                    ...
                    y[i, j1, j2, :] = a*tf.ones([1, m])
                    for k in range(d):
                        y[i, j1, j2, :] = y[i, j1, j2, :] * tf.math.pow(x[k, :], e[k])
        return y
...
class gaussians(object):
    ...
    def __call__(self, x):
        ...
        y = tf.math.exp(-1/(2*self.sigma**2)*D)
        return y
    ...
    def diff(self, x):
        ...
        for i in range(n): # for all Gaussians
            for j in range(d): # for all dimensions
                y[i, j, :] =  -2/(2*self.sigma**2) * (x[j, :] - c[j, i]) * tf.math.exp(-1/(2*self.sigma**2)*D[i, :])
    ...
    def ddiff(self, x):
        ...
        for i in range(n): # for all Gaussians
            for j1 in range(d): # for all dimensions
                for j2 in range(d): # for all dimensions
                    if j1 == j2:
                        y[i, j1, j2, :] = ( -2/(2*self.sigma**2) + 4/(4*self.sigma**4) * (x[j1, :] - c[j1, i])**2 ) * tf.math.exp(-1/(2*self.sigma**2)*D[i, :])
                    else:
                        y[i, j1, j2, :] = ( 4/(4*self.sigma**4) * (x[j1, :] - c[j1, i]) * (x[j2, :] - c[j2, i]) ) * tf.math.exp(-1/(2*self.sigma**2)*D[i, :])
        return y
