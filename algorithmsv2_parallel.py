...
class algos:
    ...
    def REINFORCE(self, f, env, reward, sigma, step_size=1/200):
        """
        ...
        #! Only here for our specific cartpole problem
        # w_r = np.array([
        #     [1],
        #     [0],
        #     [np.pi],
        ...
        for episode in range(num_episodes):
            ...
            # x = np.random.rand(self.X.shape[0],1) * state_range * np.random.choice([-1,1], size=(self.X.shape[0],1)) # random initial state
            # perturbation = np.array([
            #     [0],
            #     [0],
            #     [np.random.rand() * 0.2 * np.random.choice([-1,1])],
            #     [0]
            # ])
            # x = w_r + perturbation
            ...
            for step in range(num_steps_per_episode):
                u = pi(x, theta)
                phi_x = self.phi(x)
                x = self.tensor.B.T @ self.tensor.K_(u) @ phi_x
                x_path[:,step+1] = x[:,0]
                u_path[:,step] = u
            ...
            for step in range(num_steps_per_episode):
                ...
                phi_x = self.phi(x)
                G = 0
                for k in range(step+1, num_steps_per_episode):
                    G += self.gamma**(k-step-1) * reward(x_path[:,k], u_path[0,k])#[0,0]
                theta = theta + (step_size * np.power(self.gamma, step) * G * (((u - (phi_x.T @ theta)) * phi_x) / sigma_squared)) #* Using Gaussian policy
