...
def watch_agent(num_episodes, num_steps_per_episode, specified_episode):
    ...
    lqr_potentials = np.zeros((num_episodes, num_steps_per_episode))
    ...
    value_iteration_potentials = np.zeros_like(lqr_potentials)
    ...
    for episode_num in range(num_episodes):
        ...
        for step_num in range(num_steps_per_episode):
            ...
            lqr_potentials[episode_num, step_num] = potential(lqr_state)
            value_iteration_states[episode_num, step_num] = value_iteration_state[:, 0]
            value_iteration_potentials[episode_num, step_num] = potential(value_iteration_state)
    ...
    # Show/save plots
    save_figure("double_well_dynamics_with_value_iteration_vs_lqr")
    # show_plot()
