...
def watch_agent(num_episodes, num_steps_per_episode, specified_episode=None):
    ...
    uncontrolled_potentials = np.zeros((num_episodes, num_steps_per_episode))
    ...
    controlled_potentials = np.zeros_like(uncontrolled_potentials)
    ...
    for episode_num in range(num_episodes):
        ...
        for step_num in range(num_steps_per_episode):
            ...
            uncontrolled_potentials[episode_num, step_num] = potential(uncontrolled_state)
            controlled_states[episode_num, step_num] = controlled_state[:, 0]
            controlled_potentials[episode_num, step_num] = potential(controlled_state)
    ...
    ax.plot3D(
        ...
        uncontrolled_potentials[specified_episode]
    )
    ax.plot3D(
        controlled_states[specified_episode, :, 0],
        controlled_states[specified_episode, :, 1],
        controlled_potentials[specified_episode]
    )
    ...
    # Show plot
    save_figure("double_well_dynamics_with_lqr")
    # show_plot()
