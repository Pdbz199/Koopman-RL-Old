...
policy = ContinuousKoopmanPolicyIterationPolicy(
    ...
    all_actions,
    cost,
    'lqr-continuous-policy-iteration.pt'
)
policy.reinforce(num_training_episodes=1000, num_steps_per_episode=10)
...
def watch_agent(num_episodes, test_steps):
    ...
    plt.legend(labels)
    plt.title("Histogram Of Actions (In Final Episode)")
    plt.show()
    ...
    plt.legend(labels)
    plt.title("Scatter Plot of Actions Per Step (In Final Episode)")
    plt.show()
