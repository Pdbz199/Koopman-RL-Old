...
class ContinuousKoopmanActorCriticPolicy:
    """
    ...
    def update_policy_model(self, log_probs, deltas):
        """
        ...
        for i, (log_prob, delta) in enumerate(zip(log_probs, deltas)):
            policy_gradient[i] = -log_prob * self.gamma**((len(deltas)-i) * self.dt) * delta
    ...
    def actor_critic(
        ...
        for episode in range(num_training_episodes):
            ...
            self.update_policy_model(log_probs, deltas)
            if (episode+1) % 250 == 0:
